...
& $Python (Join-Path $PSScriptRoot 'build_inbox_search_index.py')
if ($LASTEXITCODE) { throw 'Inbox search index build failed.' }
...
& $Python (Join-Path $PSScriptRoot 'audit_code_validation.py')
if ($LASTEXITCODE) { throw 'Code validation audit failed.' }
