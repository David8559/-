...
- 论文生产链路（2026-08-04 起）：赛题+代码+已冻结结果 → [[89-Prompts/论文写作 Agent]]（八段式+五层闭环）→ [[89-Prompts/36项审查 Agent]]（36 项自查）→ `mm_common/paper_build/build_cli.py` 生成 docx → `paper_qa.py` 14 项机器 QA → [[04-Research/08-论文写作与复现/论文交付审计清单]] 人工勾选 → 人工终审（版面/工具版本/编号）。知识库为源，skill 薄封装见 `~/.claude/skills/paper-writing/`。
